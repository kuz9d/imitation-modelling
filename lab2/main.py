...
def plot_results(processing_times):
    plt.hist(processing_times, bins=50, alpha=0.7)
    plt.title("Распределение времени обработки атак")
...
if __name__ == "__main__":
    ...
    print("Результаты 1,000,000 испытаний:")
    print(f"- Блокировано Firewall: {results[0]} ({results[0]/10000:.1f}%)")
    print(f"- Обнаружено IDS: {results[1]} ({results[1]/10000:.1f}%)")
    print(f"- Блокировано IPS: {results[2]} ({results[2]/10000:.1f}%)")
    print(f"- Успешных атак: {results[3]} ({results[3]/10000:.1f}%)")
    print(f"- Среднее время обработки: {results[4]/results[3]:.2f} сек")
    print(f"- Общее время выполнения: {results[4]/3600:.2f} ч")
    print(f"- Время выполнения программы: {end-start:.2f} сек")
